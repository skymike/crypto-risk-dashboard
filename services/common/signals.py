...
def compute_market_stress(pair: str):
    ...
    regime, bias = "Unknown", "Neutral"
    long_prob = short_prob = 0.5
    ...
    if not oi.empty and not fr.empty and not sent.empty:
        ...
        if (oi_pct is not None) and oi_pct >= 90 and latest_funding < 0 and spike_ratio >= 2.0:
            regime = "Risky / High Liquidation Risk"; bias = "Short"; long_prob, short_prob = 0.25, 0.75
            ...
            if slope > 0 and latest_funding >= 0:
                regime = "Constructive"; bias = "Long"; long_prob, short_prob = 0.65, 0.35
                summary = "Upward momentum + non-negative funding → modest long bias."
            elif slope < 0 and latest_funding <= 0:
                regime = "Weak"; bias = "Short"; long_prob, short_prob = 0.35, 0.65
                summary = "Downward momentum + non-positive funding → modest short bias."
            else:
                regime = "Balanced / Choppy"; bias = "Flat"; long_prob = short_prob = 0.5
...
def compute_all_signals():
    dfpairs = fetch_df("SELECT DISTINCT pair FROM candles")
    out = []
    for p in dfpairs.get("pair", []):
        s = compute_market_stress(p); out.append(s)
...
def latest_signals_for_pairs(pairs: list[str]):
    ...
    q = f"""SELECT DISTINCT ON (pair) pair, ts, regime, bias, long_prob, short_prob, summary
             FROM signals WHERE pair IN ({placeholders}) ORDER BY pair, ts DESC"""
    df = fetch_df(q, pairs)
    result = {}
